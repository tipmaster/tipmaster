...
sub printgames {
  ...
  my $ergstring = $cllib->readFormular("G".$horr);
  ...
  for (my $sptag = $horr*3-2; $sptag <= $horr*3; $sptag++) {
	...
	$spid = $spcnt*10+$grp;
	$tiplink = "<img id=\"expbutton".$spid."\" border=\"0\" onclick=\"loadXMLDoc(".$spid.")\" src=\"/img/expand.gif\">
<img id=\"colbutton".$spid."\" border=\"0\" style=\"display:none\" onclick=\"closeXMLDoc(".$spid.")\" src=\"/img/collapse.gif\">"; 
      } else {
	$tiplink = "&nbsp;";
      }      
      ...
      print "<tr id=\"tipsrow",$spid,"\" style=\"display:none\"><td>\n";
	$gamehtml = &getTipTable($heim,$gast,$ergstring);
	print $gamehtml;
      print"</td></tr>\n";
    }
  }
...
sub getTipTable {
  ...
  my $team1 = $grptms[$id1]->team;
  my $team2 = $grptms[$id2]->team;

  my $name1 = $cllib->team2trainer("$team1");
  my $name2 = $cllib->team2trainer("$team2");
  ...
  my $toret = "<br><TABLE border=0 width=50% class=\"gametable\">\n";
  $toret .= "<TR><TD align=center colspan=3> ",$ffv,"<font size=+4>",$cllib->p2t($pkt1),":",$cllib->p2t($pkt2),"</font>",$ffv,"  &nbsp; [ $pkt1 - $pkt2 ] $ffvo<br></TD></TR>\n";
  $toret .= "<TR><TD colspan=3>&nbsp;</TD></TR>\n";
  $toret .= "<TR> <TD align=right> ",$ffv,$gamestring1,$ffvo,"</TD><TD align=center>&nbsp;</TD> <TD align=left>",$ffv,$gamestring2,$ffvo,"</TD></TR>\n";
  $toret .= "</table>\n";
