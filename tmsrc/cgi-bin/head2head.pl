...
print "<br><font face=verdana size=2><b>Bilanz $gl_vereinsname[$id1] - $gl_vereinsname[$id2] <font color=darkred> &nbsp; &nbsp; $bilanz[0] - $bilanz[1] - $bilanz[2]</b><br><br>";
...
$string = $gl_vereinsname[$tmp[0]].'#'.$bilanz[0]. ' - '.$bilanz[1].' - '.$bilanz[2]. '#' .$gl_vereinsname[$tmp[0]].'#'.$tmp[0].'#'.($sp-1000);
push(@array,$string);
}
close(A);

@arrays=(sort @array);
print "<form action=head2head.pl method=post><input type=hidden name=id1 value=$id1><input type=hidden name=loc value=$location>";
...
foreach (@gg) {
$t++;$color="#eeeeee";if($t%2 == 0){$color="white"}
@tmp=split(/#/,$_);
