...
<html><head>
   ...
   <script src="/js/tipabgabe.js"></script></head>
END_HEAD
...
print "<!-- x ist $x, gegner ist $gegner[$x]  data is $data[$gegner[$x]]] status $status[$gegner[$x]] verplatz ",$verein_platz{$data[$gegner[$x]]}," //-->\n";
...
for ($x=$spielrunde_ersatz ; $x <=$sp_bis ; $x++ ){
	...
	print "<TR><TD colspan=4><span class=\"tipheaders\">Round $x vs. <b>",$gegnerverein,"</b> (",$gegnerplatz,".) <i>",$gegnername;
	print "</i>&nbsp;<img src=\"/img/icons/other/",$imgloc,"\"/></span>";
	print "</td></TR></table>\n";
}
	
print <<END_TWO;
	<form id="postform" action="http://www.tipmaster.de/cgi-bin/tmi/formmail.pl" method=post>
