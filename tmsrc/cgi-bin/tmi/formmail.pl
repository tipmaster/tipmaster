...
sub return_html {
		...
		# Modified by Bodo for tracing
if ($create_cookie) {
   print CGI::header(-cookie=>[$cookienew]);
} else {
   print $query->header;
...
$datei = '/tmdata/tmi/tips/' . $spielrunde_ersatz . '/'.$suche .'.txt' ;
...
print "<br><br>\n";
print "<form name=daten method=post action=/cgi-bin/tmi/daten/daten.pl target=_top>
</form>\n";
...
print "<font face=verdana size=2 color=black><b>\n";
print "Vielen Dank $trainer fuer Ihre Tipabgabe<br>fuer Ihren Verein $data[$verein] !</b><br><font size=1><br>Bis Freitag 18.oo Uhr 
koennen Sie ihren Tip noch aendern .<br>\n";
...
print "<font face=verdana size=1 color=black><br>&nbsp;Auch keine Tipabgabe fuer relevante<br>&nbsp;und anstehende Pokalrunden 
vergessen ? <br>

<br>&nbsp;<font size=2><a href=/cgi-bin/cup_time.pl target=_new>Blick auf den Rahmenterminkalender</a> <br><br>";

if ( $mail_ja == 1 ) {
print "<font size=1>&nbsp;Ihr Tip wurde Ihnen auch an folgende E-Mail Adresse gemailt :<br><font color=darkred>&nbsp;$mail_adresse
<font color=black>&nbsp;&nbsp; [ -> <a href=javascript:document.daten.submit()>E-Mail Adresse aendern</a> ]<br><br>";
}
print "
&nbsp;<font color=darkred>Achtung : Bitte besuchen Sie nach Ihrer Tipabgabe erneut den<br>&nbsp;Tipabgabe - Bereich 
um zu pruefen ob Ihr Tip registriert wurde .<br><br>";



print"<br><form method=post action=/cgi-mod/tmi/login.pl target=_top>
<input type=image src=/img/men_sta.gif border=0></form>\n";
...
$mail{Message} =  "*** Tipabgabe TipMaster international [ Tiprunde $rrunde / 9 ] ***        http://www.tipmaster.de/tmi/ \n\n";
...
$mail{Message} .=  "---------------------------------------------------\n";
for ( $x=1 ; $x <= $aa; $x++ ) {
if ( $pre1[$x] == 1 ) { $aa1+=$qu_1[$sp1[$x]]; $mail{Message} .=  "Tip $pre1[$x] | Quote $qu_1[$sp1[$x]] | $paarung[$sp1[$x]] \n" }
if ( $pre1[$x] == 0 ) { $aa1+=$qu_0[$sp1[$x]]; $mail{Message} .=  "Tip $pre1[$x] | Quote $qu_0[$sp1[$x]] | $paarung[$sp1[$x]] \n" }
if ( $pre1[$x] == 2 ) { $aa1+=$qu_2[$sp1[$x]]; $mail{Message} .=  "Tip $pre1[$x] | Quote $qu_2[$sp1[$x]] | $paarung[$sp1[$x]] \n" }
}
$mail{Message} .=  "---------------------------------------------------\n";
$tmp=$aa1;
if ( $tmp > 14 ) { $dd=15; $tora = 1 }if ( $tmp > 39 ) { $dd=40; $tora = 2 }if ( $tmp > 59) {  $dd=60; $tora = 3 }if ( $tmp > 79 ) { $dd=80; $tora = 4 }if ( $tmp > 104 ) { $dd=105; $tora = 5 }if ( $tmp > 129 ) { $dd=130; $tora = 6 }if ( $tmp > 154 ) { $dd=155; $tora = 7 }
...
$mail{Message} .=  "---------------------------------------------------\n";
for ( $x=1 ; $x <= $ab; $x++ ) {
if ( $pre2[$x] == 1 ) { $aa2+=$qu_1[$sp2[$x]]; $mail{Message} .=  "Tip $pre2[$x] | Quote $qu_1[$sp2[$x]] | $paarung[$sp2[$x]] \n" }
if ( $pre2[$x] == 0 ) { $aa2+=$qu_0[$sp2[$x]]; $mail{Message} .=  "Tip $pre2[$x] | Quote $qu_0[$sp2[$x]] | $paarung[$sp2[$x]] \n" }
if ( $pre2[$x] == 2 ) { $aa2+=$qu_2[$sp2[$x]]; $mail{Message} .=  "Tip $pre2[$x] | Quote $qu_2[$sp2[$x]] | $paarung[$sp2[$x]] \n" }
}
$mail{Message} .=  "---------------------------------------------------\n";
$tmp=$aa2;
if ( $tmp > 14 ) { $dd=15; $tora = 1 }if ( $tmp > 39 ) { $dd=40; $tora = 2 }if ( $tmp > 59) {  $dd=60; $tora = 3 }if ( $tmp > 79 ) { $dd=80; $tora = 4 }if ( $tmp > 104 ) { $dd=105; $tora = 5 }if ( $tmp > 129 ) { $dd=130; $tora = 6 }if ( $tmp > 154 ) { $dd=155; $tora = 7 }
...
$mail{Message} .=  "---------------------------------------------------\n";
for ( $x=1 ; $x <= $ac; $x++ ) {
if ( $pre3[$x] == 1 ) { $aa3+=$qu_1[$sp3[$x]]; $mail{Message} .=  "Tip $pre3[$x] | Quote $qu_1[$sp3[$x]] | $paarung[$sp3[$x]] \n" }
if ( $pre3[$x] == 0 ) { $aa3+=$qu_0[$sp3[$x]]; $mail{Message} .=  "Tip $pre3[$x] | Quote $qu_0[$sp3[$x]] | $paarung[$sp3[$x]] \n" }
if ( $pre3[$x] == 2 ) { $aa3+=$qu_2[$sp3[$x]]; $mail{Message} .=  "Tip $pre3[$x] | Quote $qu_2[$sp3[$x]] | $paarung[$sp3[$x]] \n" }
}
$mail{Message} .=  "---------------------------------------------------\n";
$tmp=$aa3;
if ( $tmp > 14 ) { $dd=15; $tora = 1 }if ( $tmp > 39 ) { $dd=40; $tora = 2 }if ( $tmp > 59) {  $dd=60; $tora = 3 }if ( $tmp > 79 ) { $dd=80; $tora = 4 }if ( $tmp > 104 ) { $dd=105; $tora = 5 }if ( $tmp > 129 ) { $dd=130; $tora = 6 }if ( $tmp > 154 ) { $dd=155; $tora = 7 }
...
for ( $x=1 ; $x <= $ad; $x++ ) {
if ( $pre4[$x] == 1 ) { $aa4+=$qu_1[$sp4[$x]]; $mail{Message} .=  "Tip $pre4[$x] | Quote $qu_1[$sp4[$x]] | $paarung[$sp4[$x]] \n" }
if ( $pre4[$x] == 0 ) { $aa4+=$qu_0[$sp4[$x]]; $mail{Message} .=  "Tip $pre4[$x] | Quote $qu_0[$sp4[$x]] | $paarung[$sp4[$x]] \n" }
if ( $pre4[$x] == 2 ) { $aa4+=$qu_2[$sp4[$x]]; $mail{Message} .=  "Tip $pre4[$x] | Quote $qu_2[$sp4[$x]] | $paarung[$sp4[$x]] \n" }
}
$mail{Message} .=  "---------------------------------------------------\n";
$tmp=$aa4;
if ( $tmp > 14 ) { $dd=15; $tora = 1 }if ( $tmp > 39 ) { $dd=40; $tora = 2 }if ( $tmp > 59) {  $dd=60; $tora = 3 }if ( $tmp > 79 ) { $dd=80; $tora = 4 }if ( $tmp > 104 ) { $dd=105; $tora = 5 }if ( $tmp > 129 ) { $dd=130; $tora = 6 }if ( $tmp > 154 ) { $dd=155; $tora = 7 }
...
$mail{Message} .=  "\n\nNutzen Sie den spannenden Live - Service beim TipMaster !\n";
...
print MAIL "Subject: [TMI] Tipabgabe $topic [ Sp. $sp1 bis $sp2 ]\n\n" ;
...
################### Bodo Subroutines for tracing ####################
sub Cookie_business {
	# see if we have one
	%cookiesTrace = fetch CGI::Cookie;
	my $idee = $cookiesTrace{'TIP'};
	$create_cookie = 0;
	if (!$idee) {
		my $value = &creat_ts; 
		$cookienew = new CGI::Cookie(-name=>'TIP',-value=>$value,-expires=>'+2M');
		$create_cookie = 1; 
	}
    &tracecall;
}
...
sub tracecall {
	# Write our info string into log file
	open(N,">>/tmdata/tmi/abgabetrace.txt");
	flock(N,LOCK_EX);
	my $trainername = $cookiesTrace{'trainer'};
	my $now = localtime(time);
	my $ide = $cookiesTrace{'TIP'};
	my $tnn = "No trainer cookie";
	my $ide1 = "No TIP cookie";
	if( $trainername) {
		$tnn = $trainername->value();
	}
	if ($ide) {
		$ide1 = $ide->value();
	}
	print N $now,"\t",$ide1,"\t",$query->remote_host(),"\t",$tnn,"\t",$query->param('trainer'),"\t",$query->param('team'),"\t",$create_cookie,"\n";
	flock(N,LOCK_UN);
	close(N);
}
...
sub creat_ts {
	my $val = "";
	$val = $query->remote_host();
	my $nowy = localtime(time);
	my $rando = int(rand(10000));
	$val = $val."-".$rando."-".$nowy;
	return $val;	
}

################### Bodo Subroutines for tracing - Ende #################
