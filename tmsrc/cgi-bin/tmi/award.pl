...
print "<br><font face=verdana size=2>&nbsp;<b>TMI - Trainer Saison Awards</b><br><br>";
print "<form method=post action=/cgi-bin/tmi/award.pl target=_top>\n";
print "<font face=verdana size=1>";
...
print "&nbsp;&nbsp;<input type=hidden name=id value=\"$id\"><input type=submit style=\"font-family: Verdana; font-size: 11px; font-weight: normal; color: #000000;\" value=\"Tabelle laden\"></form>";
...
if ( $me == 4 ) {
print "<font size=1>&nbsp; Anmerkung: Der Optimizer Wert berechnet sich indem alle Quotenpunkte oberhalb einer Torgrenze,<br>
&nbsp; die zu
...
$datei = "/tmdata/tmi/stat_s" . $sai . ".txt";
if ( $sai == $akt_saison+1 ) {
$datei = "/tmdata/tmi/stat_at_aw$me.txt";
$grenze=0;$marker=30;}
...
#print $boh[1] . " > ". $grenze ."<br/>";

if ($boh[1] > $grenze) {
$rr++;
($trainer[$rr],$spiele[$rr],$pu_g[$rr],$s_g[$rr],$u_g[$rr],$n_g[$rr],$tp_g[$rr],$tm_g[$rr],$qp_g[$rr],$qm_g[$rr],$op_g[$rr],$nix,$sso[$rr]) = split (/&/,$_);
$sso[$rr]=~s/Saison//g;
}
}
close (D2) ;
...
if ( $sai == $akt_saison+1 ) {
print "<td class=ve bgcolor=#f5f5ff align=left><font face=verdana size=1>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Saison</td>\n";
...
$aa=$trainer[$ident[$t]];
$aa=~s/ /%20/g ;
print "<td bgcolor=#eeeeff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/tmi/trainer.pl?ident=$aa><img src=/img/h1.jpg border=0 alt=\"Trainerstatistik $trainer[$ident[$t]]\"></a>&nbsp;&nbsp;$trainer[$ident[$t]]&nbsp;&nbsp;</td>\n";
$color="black" ;
...
print"<td bgcolor=$co align=center><font face=verdana size=1> &nbsp; $saison[$sso[$ident[$t]]+5] &nbsp; </td>";
} else {

if ( $aa eq "" ) {
print"<td bgcolor=#f5f5ff align=center colspan=2><font face=verdana size=1>Trainer nicht mehr beim BTM aktiv</td>";
}else {
...
if ( $pl < 10 ) { $pl = '0' . $pl }
print "<td class=ve bgcolor=#f5f5ff align=right><font color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$trainer[$ident[$t]]}] ($pl.) &nbsp;</td>\n";
}}
...
if ( $me == 1 ) { $color="#CACBF6" } else { $color="#eeeeff"}
...
if ( $me == 2 ) { $color="#CACBF6" } else { $color="#eeeeff"}
