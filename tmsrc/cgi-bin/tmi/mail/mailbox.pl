...
&getValue($trainer,tmi_inbox,trainer);
@inbox = split(/#/,$query);
$in = scalar(@inbox);
if ( $in == 0 ) { $in = "keine" }
...
<form name=back action=/cgi-bin/tmi/mail/mailbox.pl method=post></form>
...
foreach $loop (@inbox) {
($id,$neu)=split(/&/,$loop);
print "<form action=/cgi-bin/tmi/mail/mailbox.pl name=show",$id,"in method=post><input type=hidden name=id value=$id><input type=hidden name=method value=show_inbox></form>\n";
}

print "<form action=/cgi-bin/tmi/mail/mailbox.pl method=post><input type=hidden name=method value=\"delete_inbox\">\n";
...
if ( $in ne "keine" ) {
print "<tr><td align=left bgcolor=#DEDFEC colspan=5 width=750><font face=verdana size=1> &nbsp;<input type=submit style=\"font-familiy:verdana;font-size=8px;\" value=\"-> Markierte Messages loeschen\"> &nbsp; &nbsp; [ PM = Personal Message | LM = Liga Message ]</td></tr>";
}
print "
</form></table>
</td></tr></table><br>
...
print "<form action=/cgi-bin/tmi/mail/mailbox.pl name=show",$loop,"out method=post><input type=hidden name=id value=$loop><input type=hidden name=method value=show_outbox></form>\n";
}

print "<form action=/cgi-bin/tmi/mail/mailbox.pl method=post><input type=hidden name=method value=\"delete_outbox\">\n";
...
if ( $out ne "keine" ) {
print "<tr><td align=left bgcolor=#DEDFEC colspan=5 width=750><font face=verdana size=1> &nbsp;<input type=submit style=\"font-familiy:verdana;font-size=8px;\" value=\"-> Markierte Messages loeschen\"></td></tr>";
}
print "
</table>
</td></tr></table><br></form>";
...
&getValue($trainer,tmi_adress,trainer);
@outbox = split(/&/,$query);
$out = scalar(@outbox);
if ( $out == 0 ) { $out = "keine" }
...
print "<form action=/cgi-bin/tmi/mail/mailbox.pl method=post><input type=hidden name=method value=\"delete_adress\">\n";
...
#if ( $out ne "keine" ) {
print "<tr><td align=left bgcolor=#DEDFEC colspan=2><font face=verdana size=1> &nbsp;<input type=submit style=\"font-familiy:verdana;font-size=8px;\" value=\"-> Markierte Trainer loeschen\"></td></form>";
...
<table border=0 cellpadding=0 cellspacing=0 bgcolor=black><tr><td>
<table border=0 cellpadding=3 cellspacing=1>
<form method=post action=/cgi-bin/tmi/mail/mail_sent.pl>
<input type=hidden name=message value=\"konkurrenz\">
<tr>
...
<option value=none>Empfaenger auswaehlen
<option value=none>-------------------------------------------------
";
foreach $x (@outbox) {
...
############ ROUTINE LOESCHEN POSTEINGANG MESSAGES #############
sub delete_inbox {
...
$inbox_new=~s/$id\&2\#//;
      ...
      $sql = "UPDATE trainer SET tmi_inbox='$inbox_new' where trainer='$trainer'";
      $sth = $dbh->prepare($sql);
      $sth->execute();
      $sth->finish();
...
############ ROUTINE LOESCHEN POSTAUSGANG MESSAGES #############
sub delete_outbox {
...
$outbox_new=~s/$id\#//;
      ...
      $sql = "UPDATE trainer SET tmi_outbox='$outbox_new' where trainer='$trainer'";
      $sth = $dbh->prepare($sql);
      $sth->execute();
      $sth->finish();
...
############ ROUTINE LOESCHEN ADDRESSBUCH TRAINER #############
sub delete_adress {
...
$outbox_new=~s/$id&//;
      ...
      $sql = "UPDATE trainer SET tmi_adress='$outbox_new' where trainer='$trainer'";
      $sth = $dbh->prepare($sql);
      $sth->execute();
      $sth->finish();
...
if ( $verein{$add} eq "") { $error_a="$add ist kein aktiver TMI - Trainer" }
...
$query=~s/$tausch1/$tausch2/;
      $sql = "UPDATE trainer SET tmi_inbox='$query' WHERE trainer='$trainer'";
      $sth = $dbh->prepare($sql);
      $sth->execute();
      $sth->finish();
}
...
<tr><td align=left bgcolor=#DEDFEC colspan=3><font face=verdana size=1>&nbsp;<img src=/img/mail.gif> &nbsp; Message \"$go[0]\" vom $go[3] - $go[4] &nbsp; &nbsp; &nbsp; [ <a href=javascript:document.back.submit()>Zurueck zur Message - Box</a> ] &nbsp; &nbsp; &nbsp;</td></tr>
<tr>
...
&getValue($go[2],tmi_inbox,trainer);
$status="Geloescht";
...
print "<input type=hidden name=message value=konkurrenz>\n";
