...
$query = new CGI;
$la = $query->param('land');
$liga = $query->param('liga');
if ( $liga > 0 ) {
$lb = $liga_namen[$liga];
($la,$lb)=split(/ /,$lb);
}
if ( $la eq "San" ) { $la = "San Marino" }
if ( $la eq "Nord" ) { $la = "Nord Irland" }
if ( $la eq "" ) { $la="Italien" }




open (D1,"</tmdata/tmi/pokal_erfolge.txt");
...
if ( $liga_namen[$x] =~/$la/ ) { $rrr=$x}

}

print "Content-type:text/html\n\n";
print "<title>TMI Titeltraeger</title>
...
<center><font face=verdana size=4><b>Bisherige Titeltraeger</b><br><font size=1>( seit Saison 2000'3)
<br><br><br>";
print "[ <a href=/halloffame.html>Bisherige Europapokal Titeltraeger</a> ]<br>
<br>[ <a href=/cgi-bin/btm/titel.pl>Bisherige Bundesliga - TipMaster Titeltraeger</a> ]<br>
<br>[ <a href=/cgi-bin/btm/titel.pl?special=fuxx>Bisherige Trainer - Fuxx Gewinner</a> ]<br>
";

print "<br><form action=titel.pl method=post><select name=land style=\"font-family:verdana; font-size=11px\">";
for ($x=1;$x<=203;$x++){
...
$aa="";
if ($la eq $land ) {$aa=" selected"}
print "<option value=\"$land\" $aa>Titeltraeger $land[$x]\n";
}
}
print "</select>&nbsp;&nbsp; <input type=submit value=\"Anzeigen\" style=\"font-family:verdana; font-size=11px\"></form>";
...
print "<tr align=center><td nowrap=nowrap colspan=7 align=center><font face=verdana size=2><br><img src=$a1> &nbsp; &nbsp; 
<b>$liga_namen[$x]</b> &nbsp; &nbsp; <img src=$a1><br><br></td></tr>\n";
print "<tr>";
print "<td></td><td nowrap=nowrap><font face=verdana size=1 color=darkblue> &nbsp; &nbsp; &nbsp;Meisterschaft</td>";
...
$all[3]=$liga_namen[$x];
if ( $all[3] eq "Italien Serie A" ||  $all[3] eq "England Premier League" ||  $all[3] eq "Spanien Primera Division" ||  $all[3] eq "Frankreich Premiere Division") {$temp = 4;}
if ($all[3] eq "Belgien 1.Division" || $all[3] eq "Portugal 1.Divisao" || $all[3] eq "Niederlande Ehrendivision" || $all[3] eq "Oesterreich Bundesliga" || $all[3] eq "Schweiz Nationalliga A") { $temp = 6 }
if ( $x==204 ) { $temp=6 }

for ($saison=$saison_aktuell;$saison>=$temp;$saison=$saison-1){

print "<tr>";
print "<td width=30 nowrap=nowrap><font face=verdana size=1 color=darkblue> $name_sai[$saison]</td>";
$rr=$meister[$saison];$rr=~s/ /%20/g;
$rs=$trainer[$saison];$rs=~s/ /%20/g;


$im="";
if ( $titel_dm{$meister[$saison]} > 1 ) { 
for ( $k=1;$k<=$titel_dm{$meister[$saison]};$k++){$im .= " <img src=/img/star_yellow.gif> " }
$titel_dm{$meister[$saison]}=0;
}

print "<td width=350 nowrap=nowrap><font face=verdana size=2 color=darkred> &nbsp; &nbsp; &nbsp;
<b><a class=\"type1\" href=/cgi-mod/tmi/verein.pl?ident=$rr>$meister[$saison]</a></b> $im <br><font color=black> &nbsp; &nbsp; &nbsp;<font size=1>
<a class=\"type2\" href=/cgi-mod/tmi/trainer.pl?ident=$rs>$trainer[$saison]</a></td>";
$rr=$cup1[$saison];$rr=~s/ /%20/g;
$rs=$cup2[$saison];$rs=~s/ /%20/g;

$im="";
if ( $titel_po{$cup1[$saison]} > 1 ) { 
for ( $k=1;$k<=$titel_po{$cup1[$saison]};$k++){$im .= " <img src=/img/star_yellow.gif> " }
$titel_po{$cup1[$saison]}=0;
}


print "<td width=250 nowrap=nowrap align=left><font face=verdana size=2 color=darkred>
 <b><a class=\"type1\" href=/cgi-mod/tmi/verein.pl?ident=$rr>$cup1[$saison]</a></b> $im <br><font SIZE=1 color=black><a class=\"type2\" href=/cgi-mod/tmi/trainer.pl?ident=$rs>$cup2[$saison]</a></td>";
...
print "<td nowrap=nowrap width=90 align=right valign=top><font face=verdana size=1 color=black>$kuerzel $tora : $torb </td>";
