...
print
"<br><font face=verdana size=2>&nbsp;<b>BTM - Ligenvergleich</b> &nbsp; &nbsp; &nbsp; &nbsp; <font size=1 face=verdana>[ QP = Quotenpunkte / UQP = Ueberfluessige Quotenpunkte ]<br><br>";
print "<form method=post action=/cgi-bin/btm/ligaquote.pl target=_top>\n";
print "<font face=verdana size=1>";
...
$datei = "/tmdata/btm/ligaquote.txt";
...
while (<D2>) {
	...
	if ( $boh[0] == $sai ) {
		$rr++;
		( $leer, $trainer[$rr], $sp, $q_g[$rr], $t_g[$rr], $o_g[$rr], $qp_g[$rr], $tp_g[$rr], $op_g[$rr] ) =
		  split( /#/, $_ );
	}
...
&nbsp;Liga</td>\n";
	...
	for ( $t = 1 ; $t <= $rr ; $t++ ) {
		...
		if ( ( $t <= $marker ) or ( $liga{$trainer} == $ident[$t] ) ) {
			$ein = 1;
		}
		if ( $loss == 2 ) { $ein = 0 }
...
&nbsp;$liga_namen[$ident[$t]]  &nbsp; &nbsp; 
 &nbsp; &nbsp; &nbsp; </td>\n";
			...
			for ( $x = $sai - 1 ; $x >= $sai - 8 ; $x-- ) {
				...
				print "<td class=ve bgcolor=#dddeff align=right> &nbsp; $d[$me+3][$ident[$t]][$x] &nbsp; </td>";
			}
