...
print "<br>";
print "<form method=post action=/cgi-bin/btm/ewigtab.pl target=_top>\n";
print "<font face=verdana size=1>";
...
print
"&nbsp;&nbsp<input type=hidden name=id value=\"$id\"><input type=submit style=\"font-family: Verdana; font-size: 11px; font-weight: normal; color: #000000;\" value=\"Tabelle laden\"> &nbsp; &nbsp; &nbsp;</form>";

if ( $me == 2 ) {
	print
"<font face=verdana size=1>[ Beim Relativ - Ranking werden nur Vereine mit mind. 34 Spielen in der entsprechenden Liga gewertet ]<br><br>";
}
...
while (<D2>) {
	...
	if ( $boh[1] > $grenze ) {
		$rr++;
		(
			$verein[$rr], $spiele[$rr], $pu_g[$rr], $pu_schnitt[$rr], $qu_schnitt[$rr],
			$s_g[$rr],    $u_g[$rr],    $n_g[$rr],  $tp_g[$rr],       $tm_g[$rr]
		) = split( /#/, $_ );
		$s_g[$rr]  = $s_g[$rr] * 1;
		$u_g[$rr]  = $u_g[$rr] * 1;
		$n_g[$rr]  = $n_g[$rr] * 1;
...
print
"<td class=ve bgcolor=#f5f5ff align=left colspan=1><font face=verdana size=1>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;aktueller Trainer</td>\n";
...
for ( $t = 1 ; $t <= $rr ; $t++ ) {
	...
	if ( $ein == 1 ) {
		...
		$aa =~ s/ /%20/g;
		print
"<td bgcolor=#eeeeff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/btm/verein.pl?ident=$aa><img src=/img/h1.jpg border=0 alt=\"Vereinsstatistik $verein[$ident[$t]]\"></a>&nbsp;&nbsp;$verein[$ident[$t]] &nbsp; &nbsp; &nbsp; </td>\n";

		$colore = "#eeeeff";
		if ( $sai == $ligav{ $verein[ $ident[$t] ] } ) { $colore = "#e3e4ff" }
		print
"<td bgcolor=$colore align=center><font face=verdana size=1 color=$color>&nbsp;&nbsp;$liga_kuerzel[$ligav{$verein[$ident[$t]]}]&nbsp;&nbsp;</td>\n";
		...
		print
"<td bgcolor=#e3e4ff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/btm/trainer.pl?ident=$aa><img src=/img/h1.jpg border=0 alt=\"Trainerstatistik $vereinb{$verein[$ident[$t]]}\"></a>&nbsp;&nbsp;$vereinb{$verein[$ident[$t]]} &nbsp; &nbsp; </td>\n";
...
print "</table>\n";
print "</td></tr></table>\n";
print "<br>&nbsp;<font face=verdana size=1>[ Es wurden alle Spiele seit der Saison 1999'1 beruecksichtigt ]";
exit;
