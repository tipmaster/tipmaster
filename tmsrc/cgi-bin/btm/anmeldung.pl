...
$rr = length($voller_name);if ( $rr > 22 ) { $ein = 1 }if ( $rr < 6 ) {$ein = 1 }
$rr = length($vorname);if ( $rr < 3 ) {$ein = 1 }
...
close (D2) ;
   ...
   $mail{Message} .= "vielen Dank fuer Ihre Registrierung auf der BTM - Warteliste. Sobald neue Vereine frei werden\n";
...
print "<option value=\"$auswahl_verein[$x]\"$sel> $auswahl_verein[$x] ( $liga_namen[$auswahl_liga[$x]] / $rang{$auswahl_verein[$x]}. Platz )\n";
}
