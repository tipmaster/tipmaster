...
<input type=hidden name=runde value=\"$ru_zurueck\">
	...
	print "</select>";
	print
	  " &nbsp; <a href=javascript:document.vor.submit()><img src=/img/h1.jpg heigth=10 width=10 border=0></a> &nbsp; ";
	...
	if ( $trainer ne "unknown" ) {
		print
		  "<select style=\"font-family: Verdana; font-size: 11px; font-weight: normal; color: #000000;\" name=loss>";
		$gh = "";
...
"&nbsp;&nbsp<input type=hidden name=id value=\"$id\"><input style=\"font-family: Verdana; font-size: 11px; font-weight: normal; color: #000000;\"  type=submit value=\"Tabelle laden\"></form><br>";
	...
	if ( $me == 1 ) {
		print
"In dieser Tabelle werden alle Ergebnisse der letzten 100 Spieltage addiert .<br>Die Tabelle wird sortiert nach der Anzahl der erreichten Punkte .<br><br>";
	}
	if ( $me == 2 ) {
		print
"In dieser Tabelle werden alle Tipergebnisse der letzten 100 Spieltage addiert .<br>Die Tabelle wird sortiert nach der bisherigen durchschnittlichen Tipquote des Trainers.<br><br>";
	}
	if ( $me == 3 ) {
		print
"In dieser Tabelle werden alle ueberfluessigen Tipergebnisse ueber die Torgrenzen hinaus der letzten 100 Spieltage gewertet .<br>Die Tabelle wird sortiert nach den durchschnittlich niedrigsten ueberfluessigen Tipresten des Trainers.<br><br>";
	}
	if ( $me == 4 ) {
		print
"In dieser Tabelle werden alle erzielten Tipergebnisse der Gegner eines Trainers der letzten 100 Spieltage gewertet .<br>Die Tabelle wird sortiert nach der bisherigen niedrigsten durchschnittlichen Tipquote der Gegner des Trainers.<br><br>";
	}
	if ( $me == 5 ) {
		print
...
"<td bgcolor=#f5f5ff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/btm/verein.pl?ident=$aa><img src=/img/h1.jpg border=0 alt=\"Vereinsstatistik $verein{$trainer[$ident[$t]]}\"></a>&nbsp;&nbsp;$verein{$trainer[$ident[$t]]}&nbsp;&nbsp;</td>\n";
			...
			print
"<td class=ve bgcolor=#f5f5ff align=left><font color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$trainer[$ident[$t]]}]&nbsp;&nbsp;</td><!--ligaid=$liga{$trainer[$ident[$t]]}-->\n";
...
"<td bgcolor=#f5f5ff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/btm/verein.pl?ident=$aa><img src=/img/h1.jpg border=0 alt=\"Vereinsstatistik $verein{$coach[$ident[$t]]}\"></a>&nbsp;&nbsp;$verein{$coach[$ident[$t]]}&nbsp;&nbsp;</td>\n";
			...
			print
"<td class=ve bgcolor=#f5f5ff align=left><font color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$coach[$ident[$t]]}]&nbsp;&nbsp;</td><!--ligaid=$liga{$coach[$ident[$t]]}-->\n";
...
"<form name=y$tt action=/cgi-mod/btm/verein.pl method=post><input type=hidden name=ident value=\"$verein{$coach[$ident[$tt]]}\"></form><td bgcolor=#f5f5ff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=javascript:document.y$tt.submit()><img src=/img/h1.jpg border=0 alt=\"Vereinsstatistik $verein{$coach[$ident[$tt]]}\"></a>&nbsp;&nbsp;$verein{$coach[$ident[$tt]]}&nbsp;&nbsp;</td>\n";
			...
			print
"<td bgcolor=#f5f5ff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$coach[$ident[$tt]]}]&nbsp;&nbsp;</td><!--ligaid=$liga{$coach[$ident[$t]]}-->\n";
			print
...
"<td bgcolor=#f5f5ff align=left><font face=verdana size=1>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Trainer</td>\n";
	print
"<td bgcolor=#f5f5ff align=left><font face=verdana size=1>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;aktueller Verein </td>\n";
	...
	for ( $t = 1 ; $t <= $tr ; $t++ ) {
		...
		if ( $ein == 1 ) {
			...
			print
"<td bgcolor=#f5f5ff align=right NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;$t.&nbsp;</td>\n";

			print
			  "<td bgcolor=#f5f5ff align=right NOWRAP><font face=verdana size=1>&nbsp;( $p{$data[0]}.)&nbsp;</td>\n";
			$tmp = $data[0];
			$tmp =~ s/ /%20/g;
			print
"<td bgcolor=#eeeeff align=left NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;<img src=/img/$ima>&nbsp;&nbsp;<a href=/cgi-mod/btm/trainer.pl?ident=$tmp target=new><img src=/img/h1.jpg border=0 alt=\"Trainerstatistik $data[0]\"></a>&nbsp;&nbsp;$data[0]&nbsp;&nbsp;</td>\n";
			$color = "black";
			$tmp   = $verein{ $data[0] };
			$tmp =~ s/ /%20/g;
			...
			if ($tmp) {
				print
"<td bgcolor=#f5f5ff align=left NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/btm/verein.pl?ident=$tmp target=new><img src=/img/h1.jpg border=0 alt=\"Vereinsstatistik $verein{$data[0]}\"></a>&nbsp;&nbsp;$verein{$data[0]}&nbsp;&nbsp;</td>\n";

				$tmp = "";
				if ( $vplatz{ $verein{ $data[0] } } < 10 ) { $tmp = "0" }
				print
"<td bgcolor=#f5f5ff align=right NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$data[0]}] [$tmp$vplatz{$verein{$data[0]}}.]&nbsp;&nbsp;</td>\n";
			}
			else {
			...
			$co = "#eeeeff";
			if ( $me == 2 || $me == 4 ) { $co = "#CACBF6" }
			print
			  "<td bgcolor=$co align=center NOWRAP><font face=verdana size=1>&nbsp;$data[5] : $data[6]&nbsp;</td>\n";
			$co = "#eeeeff";
			if ( $me == 1 ) { $co = "#CACBF6" }
			...
			print
"<td bgcolor=#eeeeee align=right NOWRAP><font face=verdana size=1>&nbsp;$kuerzel2 $wert $kuerzel1&nbsp;</td>\n";
