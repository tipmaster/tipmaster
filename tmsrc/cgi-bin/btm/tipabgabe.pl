...
#   window.open ("/Pop6.shtml", "popup", "width=550,height=340,menubar=no,locationbar=no,status=no,resizable=no,screenX=30,screenY=30");
#</script>
#';
...
sub daten_lesen {
	...
	for ( $xt = $spielrunde_ersatz - 1 ; $xt <= $spielrunde_ersatz + 2 ; $xt++ ) {
		$status[ $gegner[$xt] ] = "Tipp steht aus";
		...
		if ( -e "$datei" ) { $status[ $gegner[$xt] ] = "Tipp eingegangen" }
	}
	...
	for ( $x = 1 ; $x <= 25 ; $x++ ) {
		if ( $toto_1[$x] < 10 )   { $toto_1[$x] = '0' . $toto_1[$x] }
		if ( $toto_1[$x] < 100 )  { $toto_1[$x] = '0' . $toto_1[$x] }
		if ( $toto_1[$x] < 1000 ) { $toto_1[$x] = '0' . $toto_1[$x] }

		if ( $toto_2[$x] < 10 )   { $toto_2[$x] = '0' . $toto_2[$x] }
		if ( $toto_2[$x] < 100 )  { $toto_2[$x] = '0' . $toto_2[$x] }
		if ( $toto_2[$x] < 1000 ) { $toto_2[$x] = '0' . $toto_2[$x] }

		if ( $toto_3[$x] < 10 )   { $toto_3[$x] = '0' . $toto_3[$x] }
		if ( $toto_3[$x] < 100 )  { $toto_3[$x] = '0' . $toto_3[$x] }
		if ( $toto_3[$x] < 1000 ) { $toto_3[$x] = '0' . $toto_3[$x] }
...
"<html><body bgcolor=white text=black link=blue vlink=blue><title>Tippabgabe $coach  Sp. $spielrunde_ersatz bis $hh</title>";
	...
	print
"<br><font face=verdana size=2 color=black>&nbsp;<b>Aktuelle Tippabgabe Sp. $rs bis $hh - $data[$id] [ $datb[$id] ] <br><br></b>";
...
";
	...
	print
"<font face=verdana size=1 color=darkred>Vor der Tippabgabe nochmal kurz die letzten Resultate bzw. Tabellenposition der Vereine checken ?<br><font color=black>
Unter <a href=http://www.fussball-liveticker.eu target=new11>http://www.fussball-liveticker.eu</a> gibts Ergebnisse , 
...
<tr><td align=center bgcolor=#DEDFEC colspan=6><font face=verdana size=1>Anstehende Spiele Ihres Vereins $data[$id] fuer die aktuelle $gg.Tiprunde</td>
...
";
	...
	for ( $x = $spielrunde_ersatz - 1 ; $x <= $rr ; $x++ ) {
		...
		print "<tr><td align=left bgcolor=#eeeeee><font face=verdana size=1>&nbsp; Sp. $ss&nbsp;&nbsp;</td>
<td align=left bgcolor=#eeeeff><font face=verdana size=1>&nbsp;&nbsp;&nbsp; $ort[$x] &nbsp;&nbsp;&nbsp;</td>\n
<td align=left bgcolor=#eeeeff>&nbsp;&nbsp;
<a href=/cgi-mod/btm/verein.pl?li=$liga&ve=$gegner[$x] target=new><img src=/img/h1.jpg border=0></a>
&nbsp;&nbsp;<font face=verdana size=1> $data[$gegner[$x]]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
<form method=post action=/cgi-mod/btm/trainer.pl target=new><input type=hidden name=ident value=\"$datb[$gegner[$x]]\"><td align=left bgcolor=#eeeeee><font face=verdana size=1>&nbsp;&nbsp;&nbsp;<input type=image src=/img/h1.jpg border=0>&nbsp;&nbsp; $datb[$gegner[$x]]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></form><td align=right bgcolor=#eeeeff><font face=verdana size=1>&nbsp;&nbsp;&nbsp;( $verein_platz{$data[$gegner[$x]]}.)&nbsp;&nbsp;&nbsp;</td><td align=center bgcolor=#eeeeee><font face=verdana size=1>&nbsp;&nbsp;&nbsp;$status[$gegner[$x]]&nbsp;&nbsp;&nbsp;</td> 

<td align=center bgcolor=#eeeeff><font face=verdana size=1> &nbsp; 
<a href=/cgi-bin/head2head.pl?loc=btm&id1=$gl_vereinsid{$data[$id]}&id2=$gl_vereinsid{$data[$gegner[$x]]} target=bilanz> 
$bilanz[0] - $bilanz[1] - $bilanz[2] </a>&nbsp; </td>
	...
	print
"<font face=verdana size=1 color=black><br>&nbsp;&nbsp; Beim Click auf die Tipformular Paarungen erhalten Sie interessante Statistiken zu dem entspr. Spiel angezeigt .";
	print
"<font size=1><br>&nbsp;&nbsp; Auch keine Tippabgabe fuer relevante und anstehende Pokalrunden vergessen ? <a href=/cgi-bin/cup_time.pl target=_new>Blick auf den Rahmenterminkalender</a>";
...
<td align=center><font face=versdana size=1>155 - ..... </td></tr></table>
	...
	print "<form name=tip action=/cgi-bin/btm/formmail.pl method=post>";
...
## Live-Resultate Syndication ##############
		...
		#quick fix amateru teams
		if ( $paarung[$x] =~ /\(A\)/ ) { $t1{$verein_k1} = "" }

		if ( $t1{$verein_k1} ne "" && $t1{$verein_k2} ne "" ) {
			$paarung[$x] =
			  "<!--a href=javascript:document.xr$x.submit()-->" . $res[0] . "  - " . $res[1] . " " . "</a>";
		}
#############################################
		...
		if ( $flagge[$x] < 3 && $flagge[$x] > 0 ) {
			...
			print
"<td align=left bgcolor=$farbe>&nbsp;&nbsp;&nbsp;&nbsp;<font face=verdana size=1><img HEIGHT=10 WIDTH=14 src=/img/$flag border=0>&nbsp;&nbsp;&nbsp;$paarung[$x]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>\n";

		}
		elsif ( ( $flagge[$x] == 3 ) or ( $flagge[$x] == 5 ) or ( $flagge[$x] == 4 ) or ( $flagge[$x] == 8 ) ) {
			print
"<td align=left bgcolor=$farbe nowrap>&nbsp;&nbsp;&nbsp;&nbsp;<font face=verdana size=1><img HEIGHT=10 WIDTH=14 src=/img/$flag border=0>&nbsp;&nbsp;&nbsp;$paarung[$x]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>\n";
...
<br>";
	...
	print
"<font face=verdana size=1 color=black><br>Beim Click auf die Laenderflaggen werden die entsprechenden realen Ligatabellen geladen .<br>Nach dem Absenden des Formulars unbedingt auf die Antwortseite warten !<br><br>Die Tippabgabe ist jeweils bis Freitags 18.oo Uhr moeglich !\n";
	if ( $tip_eingegangen == 0 ) { $ab = "Tippabgabe senden" }
	if ( $tip_eingegangen == 1 ) { $ab = "Tippabgabe senden" }

	print "<br><br>\n";
	if ( $tip_status == 1 ) {

		if ( $trainer ne "Gast Zugang" ) {
			print "<input type=submit value=\"$ab\"></form></html>\n";
		}
		else {
			print "Das Sie als Gast eingeloggt sind koennen Sie das Tippabgabeformular nicht versenden .</form>";
		}

	}
	if ( $tip_status == 2 ) {
		print
"Der Tippabgabetermin ist bereits abgelaufen .<br>Es ist keine Abgabe bzw. Aenderung Ihres Tips mehr moeglich .</html>\n";
	}
...
<html>
 <head>
  <title>BTM - Tippabgabe : Kein Verein ausgewaehlt</title>
 </head>
...
<font color=red>
Bei ihrer Tippabgabe ist ein Fehler aufgetreten .<br><br><br>
<font color=black size=2>
+++ Sie haben keinen Verein ausgewaehlt +++<br><br><br>
</b></b></b><font color=black face=verdana size=1>Bitte kehren Sie zur Tippabgabe zurueck <br>
...
<html>
 <head>
  <title>BTM - Tippabgabe : Keine E-Mail Adresse angegeben</title>
 </head>
 <body bgcolor=#eeeeee text=#000000>
<p align=left><body bgcolor=white text=black>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href=http://www.vierklee.at target=_top>
...
<html>
 <head>
  <title>BTM - Tippabgabe : Kein Trainername angegeben</title>
 </head>
 <body bgcolor=#eeeeee text=#000000>
<p align=left><body bgcolor=white text=black>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href=http://www.vierklee.at target=_top>
