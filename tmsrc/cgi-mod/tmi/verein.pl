...
my @pokalname = (
	...
	"<font color=green>Pokalsieg",
	"<font color=gray>keine Daten"
);
...
print
"<form name=tr method=post action=/cgi-mod/tmi/trainer.pl target=_top><input type=hidden name=ident value=\"$datb[$verein]\"></form>\n";
...
print
"<font face=verdana size=2>&nbsp;<b>Trainer <a href=javascript:document.tr.submit()>$datb[$verein]</a></b><br>&nbsp;\n";

for ( $y = 1 ; $y <= $url_nr ; $y++ ) {
	if ( $data[$verein] eq $vs_ver[$y] ) {
		print
		  "<font face=verdana size=1>&nbsp;Vereinsseite <a href=$vs_url[$y] traget=new>$vs_ver[$y]</a> <br><br><br>";
	}
}
...
for ( $c = $saison_aktuell + 1 ; $c > 0 ; $c = $c - 1 ) {
	...
	if ( ( $sai[ $c + $offset ] != 99 ) and ( $sai[ $c + $offset ] != 0 ) ) {
		...
		print
"<td align=left bgcolor=$color><font face=verdana size=1>&nbsp;$sar[$sat[$c+$offset]] &nbsp; &nbsp; &nbsp; </td>";
		$rr = "";
		if ( $sai[ $c + $offset ] < 10 ) { $rr = "0" }
		print
"<td align=left bgcolor=#dddeff><font face=verdana size=1>&nbsp;&nbsp;&nbsp;Platz $rr$sai[$c+$offset] &nbsp;&nbsp;&nbsp;</td>";
		...
		print "<td align=center bgcolor=#cfd0e4><font face=verdana size=1>&nbsp;&nbsp;<img src=/img/pfeil"
		  . $string_flag
		  . ".gif border=0>&nbsp;&nbsp;</td>";
...
for ( $c = $main_nr - 4 ; $c > 8 ; $c-- ) {
	...
	print
"<td align=center bgcolor=$color><font face=verdana size=1>&nbsp;&nbsp;&nbsp;$pokalname[$poki[$c+3]] &nbsp;&nbsp;&nbsp;</td>
";
	if ( $cl[ $c + 3 ] eq "" ) { $cl[ $c + 3 ] = "---" }
...
for ( $x = 1 ; $x <= $wechsel ; $x++ ) {
	...
	if ( $bongo[2] ne "" ) {
		...
		$ac =~ s/ /%20/g;
		print
"<td align=lef bgcolor=#E6E8FC><font face=verdana size=1>&nbsp;Von $saison[$saison_start+3] | Sp. $aa &nbsp;</td>";
		print
"<td align=lef bgcolor=#E6E8FC><font face=verdana size=1>&nbsp;Bis $saison[$saison_ende+3] | Sp. $ab &nbsp;</td>";

		print
"<td align=lef bgcolor=#eeedff><font face=verdana size=1>&nbsp;<a href=/cgi-mod/tmi/trainer.pl?ident=$ac><img src=/img/h1.jpg alt=\"Trainerprofil $bongo[2]\" border=0></a>&nbsp;&nbsp;$bongo[2] &nbsp;</td>";
...
print
"<td align=left valign=top><font face=verdana size=1>&nbsp;&nbsp;&nbsp;&nbsp;<u>Saisonverlauf $main_saison[$ss_saison]</u><br>";
...
for ( $spieltag = 0 ; $spieltag < 34 ; $spieltag++ ) {
	...
	print
"<TD class=l><a href=/cgi-mod/tmi/verein.pl?li=$liga&ve=$gegner[$spieltag]><img src=/img/h1.jpg border=0></a>&nbsp; $data[$gegner[$spieltag]]</td> \n";
	$rf = "0";
	...
	if ( $main_nr == $ss_saison ) {
		print
"<TD class=c><a href=/cgi-mod/tmi/tips.pl?li=$liga&sp=$ss&ve=$tip[$spieltag] target\"_blank\" onClick=\"targetLink('/cgi-mod/tmi/tips.pl?li=$liga&sp=$ss&ve=$tip[$spieltag]');return false\"><IMG SRC=/img/ti.jpg BORDER=0></A></td> \n";
	}
	else {
	...
	print
"<td class=r >  <a href=/cgi-bin/head2head.pl?loc=tmi&id1=$gl_vereinsid{$data[$verein]}&id2=$gl_vereinsid{$data[$gegner[$spieltag]]} target=bilanz>$return[0] - $return[1] - $return[2]</a> &nbsp; &nbsp; ";
	print "<td bgcolor=black><SPACER TYPE=BLOCK WIDTH=1 HEIGHT=1></td>\n";
	print "</tr>";
