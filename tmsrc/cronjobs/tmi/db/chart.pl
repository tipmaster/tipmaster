...
require '/tmapp/tmsrc/cronjobs/tmi/seasonchange/save.pl';
require "/tmapp/tmsrc/cgi-bin/runde.pl";
require "/tmapp/tmsrc/cgi-bin/tmi/saison.pl";

`rm -rf /tmapp/tmstatic/www/img/chart/tmi/`;
`mkdir /tmapp/tmstatic/www/img/chart/tmi/`;
...
for ( $xf = 1 ; $xf <= $tr ; $xf++ ) {
	...
	if ( $line > 0 ) {
		...
		save_chart( $my_graph, "/tmapp/tmstatic/www/img/chart/tmi/$trainer-qu-$main_kuerzel[$main_nr]" );
		...
		save_chart( $my_graph, "/tmapp/tmstatic/www/img/chart/tmi/$trainer-to-$main_kuerzel[$main_nr]" );
		...
		save_chart( $my_graph, "/tmapp/tmstatic/www/img/chart/tmi/$trainer-op-$main_kuerzel[$main_nr]" );
		...
		save_chart( $my_graph, "/tmapp/tmstatic/www/img/chart/tmi/$trainer-lp-$main_kuerzel[$main_nr]" );
		print "$coach\n";
	}
}
