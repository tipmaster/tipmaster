...
sub readIDs {
	my $saison   = shift;
	my $filename = "$data/$main_kuerzel[$saison]/UEFA_ID.dat";
	...
	my $filename = "$data/$main_kuerzel[$saison]/ID.dat";
...
sub scrutinize {
	...
	$filename = "$data/$main_kuerzel[$saison]/DATA_$round.DAT";
...
#  open(K,"<$data/$main_kuerzel[$saison]/UEFA_$round.DAT") or die "<$data/$main_kuerzel[$saison]/UEFA_$round.DAT Cannot open UEFA data file: $!";
	...
	open( K, "<$data/$main_kuerzel[$saison]/UEFA_$round.DAT" );
	while (<K>) {
		chomp;
		if ( $round eq "FI" ) {
