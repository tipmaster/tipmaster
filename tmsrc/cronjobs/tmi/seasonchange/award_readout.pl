...
`rm -r /tmdata/tmi/trainer_db`;
`mkdir -p /tmdata/tmi/trainer_db`;

print "\n\nLaeuft an ...\n";

open (AAA,">/tmdata/tmi/awards.txt");
...
$rr=0;$ar=0;$a[1]=0;$a[2]=0;$a[3]=0;$a[4]=0;$a[5]=0;
$marker=30;
$grenze = 33 ;
...
($trainer[$rr],$spiele[$rr],$pu_g[$rr],$s_g[$rr],$u_g[$rr],$n_g[$rr],$tp_g[$rr],$tm_g[$rr],$qp_g[$rr],$qm_g[$rr],$op_g[$rr]) = split (/&/,$_);
...
print AAA "#$sai#$me#1#$trainer[$ident[1]]#\n";
print AAA "#$sai#$me#2#$trainer[$ident[2]]#\n";
print AAA "#$sai#$me#3#$trainer[$ident[3]]#\n";
...
$xx="1"; ( $yy , $xx ) = split (/\./ , $qm ) ; $oo = length($xx) ;
if ($oo==0 ) { $qm = $qm . '.0' }
print T "$qm&";

$oo=0;$qm = int ( ($a[2] / $ar ) * 10 ) / 10 ;
$xx="1"; ( $yy , $xx ) = split (/\./ , $qm ) ; $oo = length($xx) ;
if ($oo==0 ) { $qm = $qm . '.0' }
print T "$qm&";

$oo=0;$qm = int ( ($a[3] / $ar ) * 100 ) / 100 ;
...
$oo=0;$op = int ( ($a[4] / $ar ) * 1000 ) / 1000 ;
$xx="111"; ( $yy , $xx ) = split (/\./ , $op ) ; $oo = length($xx) ;
if ($oo==2 ) { $op = $op . '0' }
if ($oo==1 ) { $op = $op . '00' }
if ($oo==0 ) { $op = $op . '.000' }
print T "$op&";
$oo=0;$qm = int ( ($a[5] / $ar ) * 10 ) / 10 ;
$xx="1"; ( $yy , $xx ) = split (/\./ , $qm ) ; $oo = length($xx) ;
if ($oo==0 ) { $qm = $qm . '.0' }
print T "$qm&\n";
