#!/usr/bin/perl
BEGIN {
	push @INC,('/tmapp/tmsrc/cronjobs/btm');
}
use BadAutoTip;


open(U,">/tmdata/btm/tip_status.txt");
print U "2";
close (U);

open(U,">/tmdata/btm/pokal/tip_status.txt");
print U "2";
close (U);
...
$datei_error = "/tmdata/btm/error_tips_" . $spielrunde . ".txt" ;
open(ER,">$datei_error");
$datei_save = "/tmdata/btm/history_st_" . $spielrunde . ".txt" ;
open(L,">$datei_save");
...
$dot = "/tmdata/btm/tips/$hier/" . $datei . '.txt' ;
#print "$dot<br>";
...
$u = ">>";
if ($x==1) { $u = ">" }
open (E, "$u/tmdata/btm/tips/$hier/all.txt");
print E "$hierp\n";
close (E);
}

open(U,">>/tmdata/btm/tipanzahl.txt");
print U "$tips\n";
close (U);

open(U,">/tmdata/btm/zat$hier.txt");
close (U) ;
...
## this is for making a bad tipi
my $formfile = "/tmdata/btm/formular".$rrunde.".txt";
my $badautotip = new BadAutoTip();
my $badlist = $badautotip->getBadTipList($formfile);

open (D2 , "/tmdata/btm/tips/$hier/all.txt");
while (<D2>) {
$x++;
$line[$x] = $_ ;
chomp $line[$x] ;
...
close (U) ;
if ( $datb[$nummer] eq "Gast Zugang" ) { $rw ==1 ; $papa = 1 } 
if ( $rw == 1 ) { goto nix; }

$ff = $hier - 4;
if ( $ff < 0 ) { $ff = 33 }

$datei = "/tmdata/btm/zat" . $ff . '.txt';
open(U,"$datei");
while (<U>) {
if ($_ =~ /$datb[$nummer]/) {
$ein = 1;
...
if ( $datb[$nummer] eq "Forum Moderation" ) { $ein = 0 }
...
#Trainerentlassung
## derzeit deaktiviert, problem mit blanko tipps
## bodo, 26.06.2014
if ($ein == 1){ 
...
if ( $c == 25 ) { $change[$dd]  = $change[$dd] . $rost[$c] }
		...
		## Here we forget everything and put in a really bad tip.
		my $ortstring = $ort_1[$nummer].$ort_2[$nummer].$ort_3[$nummer].$ort_4[$nummer];
		my ($badtip,$overkill) = $badautotip->createBadTip($ortstring,$badlist);
		$change[$dd] = sprintf("%04d",$nummer).",".$badtip;
...
#open (D2 , ">/tmdata/btm/tip$spielrunde_neu.txt");
#for ( $c=0 ; $c<=$dd ; $c++){
#print D2 "$change[$c]\n";
#print "$change[$c]\n";
#}
...
$xx="";
if ( $liga <10 ) { $xx="0"};
$datei ="/tmdata/btm/" . 'tipos/' . 'QU' . $xx . $liga . "S$spielrunde.TXT" ;
open(U,">$datei");
print U "\n";
$ff = 0;
