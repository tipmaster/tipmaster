...
open (AAA,">/tmdata/btm/db/wranking.txt");
require "/tmapp/tmsrc/cgi-bin/runde.pl";
...
open(D,"</tmdata/btm/db/trainer.txt");
while(<D>){
($nummer , $ve ) = split (/&/ , $_);
chomp $ve ;
$trainer[$nummer] = $ve ;
...
$g0=$g1-99;
$rr=0;
open(AAA,">/tmdata/btm/db/wranking/stat_$g1");
for ($y=1;$y<=$t;$y++){
