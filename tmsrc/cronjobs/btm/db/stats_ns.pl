...
require "/tmdata/www/cgi-bin/runde.pl";
$ac1 = ( $rrunde - 1 );
if ( $ac1 == 0 ) { $ac1 == 9 }
$ab1 = ( $ac1 * 4 ) - 3;
$ab2 = ( $ab1 + 3 );
...
open( SP, "</tmdata/btm/db/spiele_old.txt" );
while (<SP>) {

	@alles = split( /#/, $_ );

	#if ( $aktiv{$trainer[$alles[0]]} == 1 ) {
			...
			if ( $kk > 0 ) {
				...
				for ( $ss = 1 ; $ss <= $akt_saison ; $ss++ ) {
					if ( $sp_g[$ss] > 0 ) {
						...
						print $qq
"$trainer[$nr[$tr]]&$sp_g[$ss]&$pu&$sp_s_g[$ss]&$sp_u_g[$ss]&$sp_n_g[$ss]&$sp_tp_g[$ss]&$sp_tm_g[$ss]&$qp&$qm&$op&\n";
						$zahl++;
						$ever[$zahl] =
"$pu!$ss!$trainer[$nr[$tr]]&$sp_g[$ss]&$pu&$sp_s_g[$ss]&$sp_u_g[$ss]&$sp_n_g[$ss]&$sp_tp_g[$ss]&$sp_tm_g[$ss]&$qp&$qm&$op&\n";
					}
				}
...
"$trainer[$nr[$tr]]&$sp_g[8888]&$sp_g[9999]&$sp_s_g[8888]&$sp_s_g[9999]&$sp_u_g[8888]&$sp_u_g[9999]&$sp_n_g[8888]&$sp_tp_g[8888]&$sp_tm_g[8888]&$qpd$xx&$qmd$xy&$pu&$pu_old&\n";
	...
	if ( $aktiv{"$trainer[$nr[$tr]]"} == 1 ) {
		if ( $kk > 0 ) {
			...
			print DLL
			  "$trainer[$nr[$tr]]&$ff_1&$sp_g[$akt_saison-3]&$ff_2&$sp_g[$akt_saison-2]&$ff_3&$sp_g[$akt_saison-1]&";
...
`perl /tmapp/tmsrc/cronjobs/btm/seasonchange/award_readout.pl &`;
`perl /tmapp/tmsrc/cronjobs/btm/db/chart.pl &`;

#@ever_new = sort @ever;
#open(D,">/home/tipmaster/btm/stat_ever.txt");
#print D @ever_new;
#close(D);
