...
print "<tr></tr></table>";
print "<form method=post action=/cgi-bin/tmi/last100.pl target=_top>\n";
...
print "[ Zur Erklaerung: die \"zu Vert.\" Spalte gibt Auskunft der erzielten Leistungen des Trainers in der entsprechenden Kategorie in den 4 aeltesten Spieltage
<br> der letzten 100 Spieltage. Diese \"Punkte\" muessen also quasi in  der kommenden Runde verteidigt werden da sie aus der Wertung herausfallen. ]<br><br>>> zu den <a href=ranking.pl>All Time Trainer Rankings</a><br><br>";
...
print "<td bgcolor=#f5f5ff align=left><font face=verdana size=1 color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$coach[$ident[$t]]}]&nbsp;&nbsp;</td><!--ligaid=$liga{$coach[$ident[$t]]}-->\n";
...
$file = "/tmdata/tmi/db/wranking/stat_".$border;
if (!-e $file) {$border=$border+2;$file = "/tmdata/tmi/db/wranking/stat_".$border;}
open(A,$file);
...
$last=(($saison_sel-6)*34)+(($tmpr-1)*4);
$file = "/tmdata/tmi/db/wranking/stat_".$last;

if (!-e $file) {$last=$last+2;$file = "/tmdata/tmi/db/wranking/stat_".$last;}
open(A,$file);
while(<A>){
...
print "<td bgcolor=#eeeeff align=left NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;<img src=/img/$ima>&nbsp;&nbsp;<a href=/cgi-mod/tmi/trainer.pl?ident=$tmp target=new><img src=/img/h1.jpg border=0 alt=\"Trainerstatistik $data[0]\"></a>&nbsp;&nbsp;$data[0]&nbsp;&nbsp;</td>\n";
$color="black" ;
$tmp=$verein{$data[0]};$tmp=~s/ /%20/g;

if ($tmp ne "")  {

print "<td bgcolor=#f5f5ff align=left NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;<a href=/cgi-mod/tmi/verein.pl?ident=$tmp target=new><img src=/img/h1.jpg border=0 alt=\"Vereinsstatistik $verein{$data[0]}\"></a>&nbsp;&nbsp;$verein{$data[0]}&nbsp;&nbsp;</td>\n";
$tmp="";if ($vplatz{$verein{$data[0]}} < 10){$tmp="0"}
print "<td bgcolor=#f5f5ff align=right NOWRAP><font face=verdana size=1 color=$color>&nbsp;&nbsp;$liga_kuerzel[$liga{$data[0]}] [$tmp$vplatz{$verein{$data[0]}}.]&nbsp;&nbsp;</td><!--ligaid=$liga{$data[0]}-->\n";
} else {
